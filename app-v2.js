...
const state = { config: { configured: false, turnstileSiteKey: null, model: "gemini-3.5-flash-lite" }, selectedFile: null, extraction: null, plan: null, generatedHTML: "", generatedFilename: "", turnstileToken: "", turnstileId: null, busy: false };
...
function selectFile(file) { if (!file) return; state.selectedFile = file; clearResult(); el.dropZone.hidden = true; el.fileCard.hidden = false; el.fileName.textContent = file.name; const policy = getUploadPolicy(); const warning = file.size > policy.warningBytes ? " · large file; adaptive mode" : " · ready to import"; el.fileMeta.textContent = `${formatBytes(file.size)}${warning}`; if (file.size > policy.maxBytes) message(`This device's safe upload limit is ${policy.mobile ? "20" : "50"} MB.`, "warning"); updateButton(); }
...
async function requestBatch(extraction, userOptions, batch, index, total) {
  ...
  for (let attempt = 1; attempt <= 2; attempt += 1) {
    ...
    try {
      ...
      return payload.plan;
    } catch (error) { lastError = error; if (attempt < 2) await new Promise((resolve) => setTimeout(resolve, 800)); }
  }
  throw lastError;
}
...
async function processLecture() {
  ...
  state.busy = true; updateButton(); showProcessing(); let fallbackReason = "";
  try {
    ...
    if (state.config.configured) { try { state.plan = await requestPlan(state.extraction, userOptions); } catch (error) { fallbackReason = error instanceof Error ? error.message : "The AI service was unavailable."; state.plan = createFallbackPlan(state.extraction, userOptions); } finally { resetTurnstile(); } }
    else { fallbackReason = "Gemini is not configured in Cloudflare Pages, so Jang used its local preservation layout."; state.plan = createFallbackPlan(state.extraction, userOptions); }
    ...
    notices.unshift(fallbackReason ? `AI reorganization was unavailable: ${fallbackReason} A local source-order layout was created instead.` : `Gemini reorganized ${stats.batchCount} independently retryable batch${stats.batchCount === 1 ? "" : "es"}.`);
    message(notices.join(" "), "warning");
  } catch (error) { el.processingState.hidden = true; el.emptyState.hidden = false; message(error instanceof Error ? error.message : "The lecture could not be processed."); }
...
async function loadConfig() { try { const response = await fetch("/api/config", { headers: { accept: "application/json" } }); if (!response.ok) throw new Error(`HTTP ${response.status}`); state.config = { ...state.config, ...(await response.json()) }; backendStatus(state.config.configured ? "ready" : "warning", state.config.configured ? `Gemini ready · ${state.config.model}` : "Local mode · add Gemini secret"); loadTurnstile(state.config.turnstileSiteKey); } catch { backendStatus("warning", "Local mode · API unavailable"); } finally { updateButton(); } }

el.fileInput.addEventListener("change", () => selectFile(el.fileInput.files?.[0])); el.removeFile.addEventListener("click", removeFile); el.processButton.addEventListener("click", processLecture); el.downloadButton.addEventListener("click", downloadHtml); el.downloadPptxButton?.addEventListener("click", downloadPowerPoint); el.openPreviewButton.addEventListener("click", openPreview); el.closeDialog.addEventListener("click", () => el.previewDialog.close()); el.previewDialog.addEventListener("click", (event) => { if (event.target === el.previewDialog) el.previewDialog.close(); });
